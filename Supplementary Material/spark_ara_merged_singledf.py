...
featureDF = featureDF.join(bh_df, on=['id'], how='inner').dropDuplicates().repartition(repartition_count).persist()
...
def get_neighbors_mfcc_js_skl(song, fullFeatureDF):
    comparator_value = song[0]["mfccSkl"]
    distance_udf = F.udf(lambda x: float(jensen_shannon(x, comparator_value)), DoubleType())
    fullFeatureDF = fullFeatureDF.withColumn('distances_js', distance_udf(F.col('mfccSkl')))
    fullFeatureDF = fullFeatureDF.filter(fullFeatureDF.distances_js != np.inf)    
    ################################
    #to be able to do this performant mfccSkl has to be dropped -> SKL and JS right after each other
    ################################
    distance_udf = F.udf(lambda x: float(symmetric_kullback_leibler(x, comparator_value)), DoubleType())
    fullFeatureDF = fullFeatureDF.withColumn('distances_skl', distance_udf(F.col('mfccSkl')))
    #thresholding 
    #fullFeatureDF = fullFeatureDF.filter(fullFeatureDF.distances_skl <= 100)  
    fullFeatureDF = fullFeatureDF.filter(fullFeatureDF.distances_skl != np.inf).drop("mfccSkl").persist()       
...
def get_neighbors_mfcc_euclidean(song, fullFeatureDF):
    ...
    distance_udf = F.udf(lambda x: float(distance.euclidean(x, comparator_value)), FloatType())
    fullFeatureDF = fullFeatureDF.withColumn('distances_mfcc', distance_udf(F.col('mfccEuc'))).drop("mfccEuc").persist()
    return fullFeatureDF

def get_neighbors_notes(song, fullFeatureDF):
    comparator_value = song[0]["notes"]
    fullFeatureDF = fullFeatureDF.withColumn("compare", lit(comparator_value)).persist()
...
def perform_scaling(fullFeatureDF):
    aggregated = fullFeatureDF.agg(F.min(fullFeatureDF.distances_bh),F.max(fullFeatureDF.distances_bh),F.mean(fullFeatureDF.distances_bh),F.stddev(fullFeatureDF.distances_bh),
        F.min(fullFeatureDF.distances_rh),F.max(fullFeatureDF.distances_rh),F.mean(fullFeatureDF.distances_rh),F.stddev(fullFeatureDF.distances_rh),
        F.min(fullFeatureDF.distances_rp),F.max(fullFeatureDF.distances_rp),F.mean(fullFeatureDF.distances_rp),F.stddev(fullFeatureDF.distances_rp),
        F.min(fullFeatureDF.distances_corr),F.max(fullFeatureDF.distances_corr),F.mean(fullFeatureDF.distances_corr),F.stddev(fullFeatureDF.distances_corr),
        F.min(fullFeatureDF.distances_levenshtein),F.max(fullFeatureDF.distances_levenshtein),F.mean(fullFeatureDF.distances_levenshtein),F.stddev(fullFeatureDF.distances_levenshtein),
        F.min(fullFeatureDF.distances_mfcc),F.max(fullFeatureDF.distances_mfcc),F.mean(fullFeatureDF.distances_mfcc),F.stddev(fullFeatureDF.distances_mfcc),
        F.min(fullFeatureDF.distances_js),F.max(fullFeatureDF.distances_js),F.mean(fullFeatureDF.distances_js),F.stddev(fullFeatureDF.distances_js),
        F.min(fullFeatureDF.distances_skl),F.max(fullFeatureDF.distances_skl),F.mean(fullFeatureDF.distances_skl),F.stddev(fullFeatureDF.distances_skl)).persist()
    ##############################
    ...
    fullFeatureDF = fullFeatureDF.withColumn('scaled_mfcc', (fullFeatureDF.distances_mfcc-min_val)/(max_val-min_val)).select("id", "key", "scale", "bpm", "scaled_rp", "scaled_rh", "scaled_bh", "scaled_notes", "scaled_chroma", "scaled_skl", "scaled_js", "scaled_mfcc").unpersist()
    ##############################
    return fullFeatureDF

def get_nearest_neighbors(song, outname, fullFeatureDF):
    tic1 = int(round(time.time() * 1000))
    ...
    tic1 = int(round(time.time() * 1000))
    fullFeatureDF = get_neighbors_mfcc_euclidean(song, fullFeatureDF).persist()
    tac1 = int(round(time.time() * 1000))
    ...
    tic1 = int(round(time.time() * 1000))
    fullFeatureDF = get_neighbors_mfcc_js_skl(song, fullFeatureDF).persist()
    tac1 = int(round(time.time() * 1000))
    ...
    tic1 = int(round(time.time() * 1000))
    fullFeatureDF = perform_scaling(fullFeatureDF).persist()
    tac1 = int(round(time.time() * 1000))
    time_dict['SCALE: ']= tac1 - tic1

    tic1 = int(round(time.time() * 1000))
    #fullFeatureDF = fullFeatureDF.withColumn('aggregated', (fullFeatureDF.scaled_notes + fullFeatureDF.scaled_rp + fullFeatureDF.scaled_mfcc) / 3)
...
res1 = get_nearest_neighbors(song1, "MERGED_FULL_S_SONG1.csv", fullFeatureDF).persist()
tac1 = int(round(time.time() * 1000))
...
#and then calculate features
res2 = get_nearest_neighbors(song2, "MERGED_FULL_S_SONG2.csv", fullFeatureDF).persist()
...
tic1 = int(round(time.time() * 1000))
res1.toPandas().to_csv("MERGED_FULL_S_SONG1.csv", encoding='utf-8')
res1.unpersist()
tac1 = int(round(time.time() * 1000))
time_dict['CSV1: ']= tac1 - tic1

tic2 = int(round(time.time() * 1000))
res2.toPandas().to_csv("MERGED_FULL_S_SONG2.csv", encoding='utf-8')
res2.unpersist()
tac2 = int(round(time.time() * 1000))
