...
LOGFNAME=`date "+anyconnect-linux64-4.5.03040-core-vpn-webdeploy-k9-%H%M%S%d%m%Y.log"`
...
echo "Installing "${NEWTEMP}/libaccurl.so.4.4.0 >> /tmp/${LOGFNAME}
${INSTALL} -o root -m 755 ${NEWTEMP}/libaccurl.so.4.4.0 ${LIBDIR} || exit 1

echo "Creating symlink "${NEWTEMP}/libaccurl.so.4 >> /tmp/${LOGFNAME}
ln -s ${LIBDIR}/libaccurl.so.4.4.0 ${LIBDIR}/libaccurl.so.4 || exit 1
