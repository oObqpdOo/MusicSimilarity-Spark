...
import openmars.audiofile_read # reading wav and mp3 files
from openmars.rp_feature_io import CSVFeatureWriter, HDF5FeatureWriter, read_csv_features, load_multiple_hdf5_feature_files
import openmars.rp_extract as rp # Rhythm Pattern extractor
import openmars.unicsv # Rhythm Pattern extractor
import sys
...
class extractor:
    def __init__(self, path, outpath):
        ...
        self.pathName = path
        self.fs = 44100
        self.octave = ['A', 'A#', 'B', 'C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#']
        self.do_mfcc_kl = 1
        self.do_mfcc_euclid = 1
        self.do_notes = 1
        self.do_chroma = 1
        self.do_bh = 1
        self.startbatch = 0
        self.endbatch = 100000000
        self.batchsize = 25
        self.featurepath = outpath

    def extract_features(self):
        print("Selected Folder" + self.pathName)
        filelist = []
        for pathName in Path(str(self.pathName)).glob('**/*.mp3'):
            filelist.append(pathName)
        for pathName in Path(str(self.pathName)).glob('**/*.wav'):
            filelist.append(pathName)      
        for pathName in Path(str(self.pathName)).glob('**/*.aif'):
            filelist.append(pathName)   
        for pathName in Path(str(self.pathName)).glob('**/*.wma'):
            filelist.append(pathName)      
        print("length of filelist: " + str(len(filelist)))
        print("Extracting Rhythm Features") 
        self.extract_all_rhythm_feats(self.pathName, self.startbatch, self.endbatch, self.batchsize, self.size, self.rank)
        print("Extracting Melodic Features") 
        time_dict = {}
        tic1 = int(round(time.time() * 1000))
        # BATCH FEATURE EXTRACTION:
        self.process_stuff(filelist, self.startbatch, self.endbatch, self.batchsize, self.size, self.rank, self.do_mfcc_kl, self.do_mfcc_euclid, self.do_notes, self.do_chroma, self.do_bh)
        tac1 = int(round(time.time() * 1000))
        time_dict['MPI TIME FEATURE']= tac1 - tic1
        #if rank == 0:
        print("Process " + str(self.rank) + " time: " + str(time_dict)) 


    def parallel_python_process(self, process_id, cpu_filelist, f_mfcc_kl, f_mfcc_euclid, f_notes, f_chroma, f_bh):
        #return (end_time - start_time)
        #PARAMETER: mfcc_kl, mfcc_euclid, notes, chroma, bh
        if f_mfcc_euclid == 1:    
            with open(self.featurepath + "out" + str(process_id) + ".mfcc", "w") as myfile:
                myfile.write("")
                myfile.close()
        if f_mfcc_kl == 1:    
            with open(self.featurepath + "out" + str(process_id) + ".mfcckl", "w") as myfile:
                myfile.write("")
                myfile.close()
        if f_chroma == 1:
            with open(self.featurepath + "out" + str(process_id) + ".chroma", "w") as myfile:
                myfile.write("")
                myfile.close()       
        if f_bh == 1:   
            with open(self.featurepath + "out" + str(process_id) + ".bh", "w") as myfile:
                myfile.write("")
                myfile.close()
        if f_notes == 1:        
            with open(self.featurepath + "out" + str(process_id) + ".notes", "w") as myfile:
                myfile.write("")
                myfile.close()
        count = 1
        for file_name in cpu_filelist:
            ...
            bpmret, hist, key, scale, notes, chroma_matrix, mean, cov, var, cov_kl = self.compute_features(path, f_mfcc_kl, f_mfcc_euclid, f_notes, f_chroma, f_bh)
            if key == 0:
                continue
            ...
            if f_mfcc_euclid == 1:                
                with open(self.featurepath + "out" + str(process_id) + ".mfcc", "a") as myfile:
                    ...
                    myfile.close()
            if f_chroma == 1:  
                with open(self.featurepath + "out" + str(process_id) + ".chroma", "a") as myfile:
                    print ("Chroma Full - File " + path + " " + str(count) + " von " + str(len(cpu_filelist))) 
                    transposed_chroma = np.zeros(chroma_matrix.shape)
                    transposed_chroma = self.transpose_chroma_matrix(key, scale, chroma_matrix)
                    ...
                    myfile.close()
            if f_bh == 1:  
                with open(self.featurepath + "out" + str(process_id) + ".bh", "a") as myfile:
                    ...
                    myfile.close()
            if f_notes == 1:  
                with open(self.featurepath + "out" + str(process_id) + ".notes", "a") as myfile:
                    print ("Chroma Notes - File " + path + " " + str(count) + " von " + str(len(cpu_filelist))) 
                    key = str(key)
                    transposed_notes = []
                    transposed_notes = self.transpose_chroma_notes(key, scale, notes)
                    #print notes
                    ...
                    myfile.close()
            if f_mfcc_kl == 1:                
                with open(self.featurepath + "out" + str(process_id) + ".mfcckl", "a") as myfile:
    ...
    def parallel_python_process_files(self, size, rank, process_id, cpu_filelist):
        print("calling rank " + str(rank) + " size " + str(size))
        count = 1
        for file_name in cpu_filelist:
            path = str(PurePath(file_name))
            filename = path.replace(".","").replace(";","").replace(",","").replace("mp3",".mp3").replace("aiff",".aiff").replace("aif",".aif").replace("au",".au").replace("m4a", ".m4a").replace("wav",".wav").replace("flac",".flac").replace("ogg",".ogg")  # rel. filename as from find_files
            with open(self.featurepath + "out" + str(process_id) + ".files", "a") as myfile:
                #print ("File " + path + " " + str(count) + " von " + str(len(cpu_filelist))) 
    ...
    def process_stuff(self, filelist, startjob, maxparts, batchsz, size, rank, f_mfcc_kl, f_mfcc_euclid, f_notes, f_chroma, f_bh):
        ...
        for index in range(startjob + rank, maxparts, size):
            if index < parts:        
                ...
                self.parallel_python_process(index, filelist[int(starti):int(endi)], f_mfcc_kl, f_mfcc_euclid, f_notes, f_chroma, f_bh)
                with open(self.featurepath + "out" + str(index) + ".files", "w") as myfile:
                    myfile.write("")
                    myfile.close()
                self.parallel_python_process_files(self.size, self.rank, index, filelist[starti:endi])
                gc.collect()
        gc.enable()
        gc.collect()

    def transpose_chroma_matrix(self, key, scale, chroma_param):
        if key == 'Ab':
            key = 'G#'
        if key == 'Gb':
            key = 'F#'
    ...
    def transpose_chroma_notes(self, key, scale, notes):
        if key == 'Ab':
            key = 'G#'
        if key == 'Gb':
            key = 'F#'
    ...
    def compute_features(self, path, f_mfcc_kl, f_mfcc_euclid, f_notes, f_chroma, f_bh):
        gc.enable()        
        # Loading audio file
        #will resample if sampleRate is different!
        try: 
            audio = es.MonoLoader(filename=path, sampleRate=self.fs)()
        except: 
        ...
        if f_bh == 1 or f_chroma == 1 or f_notes == 1: 
            ...
            tempo = bpm
            times = beats
            beats_frames = (beats * self.fs) / hopSize
    ...
    '''
    def read_feature_files(self, filenamestub,ext,separate_ids=True,id_column=0):
    ...
    @author: 2014-2015 Alexander Schindler, Thomas Lidy
    '''
    def find_files(self, path,file_types=('.wav','.mp3'),relative_path = False,verbose=False,ignore_hidden=True):
    ...
    @author: 2014-2015 Alexander Schindler, Thomas Lidy
    '''
    def extract_all_files_generic(self, in_path, 
                                  startbatch,
                                  endbatch,
                                  batchsize, 
                                  size,
        ...
        if in_path.lower().endswith('.txt'):  # treat as input file list
            ...
            filelist = self.find_files(in_path,audiofile_types,relative_path=True)
        ...
        for index in range(int(startjob + rank), int(maxparts), int(size)):
            if index < parts:        
                ...
                print("calling process  " + str(rank) + " index " + str(index) + " size " + str(size) + " starti " + str(starti) + " endi " + str(endi))
                self.extract_all_files(filelist[starti:endi], in_path, out_file + str(index), feature_types, label, append, no_extension_check, force_resampling, out_HDF5, log_AudioTypes, log_Errors, verbose)
                gc.collect()
        gc.enable()
        gc.collect()
        return 
    ...
    @author: 2014-2015 Alexander Schindler, Thomas Lidy
    '''
    def extract_all_files(self, filelist,
                          path,
                          out_file=None,
                          feature_types =['rp','ssd','rh'],
                          label=False,
        ...
        if out_file: # only if out_file is specified
            if log_AudioTypes:
                ...
                audio_logwriter = openmars.unicsv.UnicodeCSVWriter(audio_logfile) #, quoting=csv.QUOTE_ALL)
            if log_Errors:
                #pass            
                err_log_filename = out_file + '.errors.log'
                error_logfile = open(err_log_filename, 'w') # TODO allow append mode 'a'
                error_logwriter = openmars.unicsv.UnicodeCSVWriter(error_logfile) #, quoting=csv.QUOTE_ALL)
            if out_HDF5:
        ...
        for fil in filelist:  # iterate over all files
            try:
                ...
                if path:
                    ...
                    filename = fil
                if verbose:
                    print('#',n,'/',n_files,'(ETA: ' + self.timestr(remain_time) + "):", filename)
                # read audio file (wav or mp3)
                samplerate, samplewidth, data, decoder = openmars.audiofile_read.audiofile_read(filename, verbose=verbose, include_decoder=True, no_extension_check=no_extension_check, force_resampling=force_resampling)
                # audio file info
        ...
        end_time = time.time()
        if verbose:
            print("FEATURE EXTRACTION FINISHED.", n, "file(s) processed,", n_extracted, "successful. Duration:", self.timestr(end_time-start_time))
            if err > 0:
    ...
    def extract_all_rhythm_feats(self, param_folder, 
                                  startbatch,
                                  endbatch,
                                  batchsize, 
                                  size,
        ...
        audiofile_types = openmars.audiofile_read.get_supported_audio_formats()
        output_filename = self.featurepath + "out"
        ...
        time_dict = {}
        tic1 = int(round(time.time() * 1000))
        self.extract_all_files_generic(input_path, self.startbatch, self.endbatch, self.batchsize, self.size, self.rank, output_filename, feature_types, audiofile_types,
