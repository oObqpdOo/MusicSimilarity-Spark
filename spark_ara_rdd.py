...
rh = rh.map(lambda x: x.replace(' ', '').replace(';', ','))
...
rh = rh.map(lambda x: (x[0].replace(";","").replace(".","").replace(",","").replace(" ",""), x[1].replace('[', '').replace(']', '').split(",")))
...
kv_bh = bh.map(lambda x: (x[0].replace(' ', '').replace(';', ','), x[1], Vectors.dense(x[2].replace(' ', '').replace('[', '').replace(']', '').split(','))))
...
def get_neighbors_rh_euclidean(song):
    ...
    #   Get Neighbors
    #  
    comparator = rh_vec.lookup(song.replace(' ', '').replace(';', ','))
    comparator_value = comparator[0]
...
def get_neighbors_bh_euclidean(song):
    ...
    #   Get Neighbors
    #  
    comparator = bh_vec.lookup(song.replace(' ', '').replace(';', ','))
    comparator_value = comparator[0]
...
def get_neighbors_notes(song):
    ...
    #   Get Neighbors
    #  
    comparator = notes.lookup(song.replace(' ', '').replace(';', ','))
    comparator_value = comparator[0]
...
def get_neighbors_mfcc_euclidean(song):
    ...
    #   Get Neighbors
    #
    comparator = mfcceucVec.lookup(song.replace(' ', '').replace(';', ','))
    comparator_value = Vectors.dense(comparator[0])
    resultMfcc = mfcceucVec.map(lambda x: (x[0], distance.euclidean(np.array(x[1]), np.array(comparator_value))))
