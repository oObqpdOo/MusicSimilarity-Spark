...
def get_neighbors_bh_euclidean(song):
    ...
    df = sqlContext.createDataFrame(kv_bh, ["id", "bpm", "features"]).persist()
    filterDF = df.filter(df.id == song)
