...
filename = "perf_df" + str(numsongs) + ".txt"
f= open(filename,"w+")

kv_rp = kv_rp.zipWithIndex().filter(lambda vi: vi[1] < numsongs).keys().persist()
kv_rh = kv_rh.zipWithIndex().filter(lambda vi: vi[1] < numsongs).keys().persist()
kv_bh = kv_bh.zipWithIndex().filter(lambda vi: vi[1] < numsongs).keys().persist()
notes = notes.zipWithIndex().filter(lambda vi: vi[1] < numsongs).keys().persist()
mfccDfMerged = mfccDfMerged.limit(numsongs).persist()
chromaDf = chromaDf.limit(numsongs).persist()
...
time_dict['DF_TOTAL_SONG1: ']= total2 - total1
