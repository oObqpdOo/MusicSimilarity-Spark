...
while numsongs > 10000: 
    ...
    f.write("Amount of songs %d\r\n" % numsongs)
    f.write("\r\n")
    f.write("Avg processing time of cached songs %d\r\n" % (avg_time / repetitions))
    f.write("\r\n")
    avg_time = 0
    ...
    numsongs = numsongs - 10000
    rp_vec = rp_vec.zipWithIndex().filter(lambda vi: vi[1] < numsongs).keys().persist()
    rh_vec = rh_vec.zipWithIndex().filter(lambda vi: vi[1] < numsongs).keys().persist()
    bh_vec = bh_vec.zipWithIndex().filter(lambda vi: vi[1] < numsongs).keys().persist()
    notes = notes.zipWithIndex().filter(lambda vi: vi[1] < numsongs).keys().persist()
    mfccVec = mfccVec.zipWithIndex().filter(lambda vi: vi[1] < numsongs).keys().persist()
    chromaVec = chromaVec.zipWithIndex().filter(lambda vi: vi[1] < numsongs).keys().persist()
    repeated = repetitions
