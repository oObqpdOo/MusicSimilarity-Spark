...
LOGFNAME=`/bin/date "+posture-4.5.03040-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=`/usr/bin/date "+posture-4.5.03040-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=`date "+posture-4.5.03040-install-%H%M%S%d%m%Y.log"` ||
  LOGFNAME=posture-4.5.03040-install.log
...
echo "Installing Cisco AnyConnect Posture Module v4.5.03040..." > /tmp/${LOGFNAME}
...
echo "Installing "${DIR}/libaccurl.so.4.4.0 >> /tmp/${LOGFNAME}
${INSTALL} -o root -m 755 ${DIR}/libaccurl.so.4.4.0 ${LIBDIR} || 
    ( echo "failed." && exit 1 )
