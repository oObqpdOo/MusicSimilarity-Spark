...
count=1
for i in {1..10}; do
	echo "RUN_$1" >> 500k/similarity_rh_bh_mfcc_500k${outfilenum}.txt
	spark-submit --archives pyspark_venv.tar.gz#pyspark_venv 500k/similarity_rh_bh_mfcc_500k.py >> 500k/similarity_rh_bh_mfcc_500k${outfilenum}.txt  
	((count++))
done

for i in {1..10}; do
	echo "RUN_$1" >> 500k/similarity_rh_mfcc_notes_500k${outfilenum}.txt
	spark-submit --archives pyspark_venv.tar.gz#pyspark_venv 500k/similarity_rh_mfcc_notes_500k.py >> 500k/similarity_rh_mfcc_notes_500k${outfilenum}.txt  
	((count++))
done
