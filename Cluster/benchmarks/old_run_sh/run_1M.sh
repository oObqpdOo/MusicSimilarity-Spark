...
count=1
for i in {1..10}; do
	echo "RUN_$1" >> 1M/similarity_rh_bh_mfcc_1M${outfilenum}.txt
	spark-submit --archives pyspark_venv.tar.gz#pyspark_venv 1M/similarity_rh_bh_mfcc_1M.py >> 1M/similarity_rh_bh_mfcc_1M${outfilenum}.txt  
	((count++))
done

for i in {1..10}; do
	echo "RUN_$1" >> 1M/similarity_rh_mfcc_notes_1M${outfilenum}.txt
	spark-submit --archives pyspark_venv.tar.gz#pyspark_venv 1M/similarity_rh_mfcc_notes_1M.py >> 1M/similarity_rh_mfcc_notes_1M${outfilenum}.txt  
	((count++))
done
